...
from everwas.schemas.alert import (
    SECRET_CONFIG_KEYS,
    AlertOut,
    AlertRuleIn,
    AlertRuleOut,
...
from everwas.services.outbox import outbox_health
...
async def test_channel(channel_id: uuid.UUID, db: DbSession, user: CurrentUser) -> OutboxOut:
    ...
    entry = NotificationOutbox(
        ...
        payload={
            "kind": "test",
            "title": "Everwas test notification",
...
async def outbox_health_view(db: DbSession, _user: CurrentUser) -> dict:
    ...
    everwas.services.outbox.outbox_health.
    """
