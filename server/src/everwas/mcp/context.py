...
Authentication is an Everwas API key (`ewpk_<key_id>_<secret>`) presented as a
bearer token. `everwas.mcp.server.ApiKeyVerifier` turns that token into a
...
# Key verification moved to everwas.security.api_keys when the sync API
# became the second surface that takes these keys. Re-exported so MCP code
# and its tests keep their import path.
from everwas.security.api_keys import (  # noqa: F401
    KEY_PREFIX,
    ApiKeyPrincipal,
    authenticate,
    hash_secret,
...
CLAIM_KEY_ID = "everwas_key_id"
CLAIM_KEY_NAME = "everwas_key_name"
...
def current_principal() -> ApiKeyPrincipal:
    ...
    if token is None:
        raise ToolError(
            "Unauthenticated. Present an Everwas API key as a bearer token "
            "(Authorization: Bearer ewpk_...)."
        )
    claims = token.claims or {}
    key_id = claims.get(CLAIM_KEY_ID)
    if not key_id:
        raise ToolError("This token was not issued by Everwas. Use an Everwas API key.")
...
def require_scope(principal: ApiKeyPrincipal, scope: str) -> None:
    if not principal.has(scope):
        ...
        raise ToolError(
            ...
            f"Scopes on this key: {held}. Ask an Everwas administrator for a key "
