...
cli = typer.Typer(help="Everwas server administration", no_args_is_help=True)
...
def gen_enrollment_token(max_uses: int = 1, ttl_hours: int = 24, created_by: str = "cli") -> None:
    ...
    from everwas.db.engine import session_scope
    from everwas.services.enrollment import mint_enrollment_token
...
def gen_nats_keys() -> None:
    ...
    from everwas.natsio.jwt import generate_account_keypair

    seed, public = generate_account_keypair()
    typer.echo(f"EVERWAS_NATS_AUTH_SEED={seed}")
    typer.echo(f"EVERWAS_NATS_AUTH_ISSUER={public}")
