...
def subject_for(note: Notification) -> str:
    return f"[OpenRMM][{note.severity}] {note.title}"
...
def build_message(note: Notification, to: list[str], sender: str) -> EmailMessage:
    ...
    message["X-OpenRMM-Kind"] = note.kind
    message["X-OpenRMM-Severity"] = note.severity
    if note.alert_id:
        message["X-OpenRMM-Alert"] = note.alert_id
