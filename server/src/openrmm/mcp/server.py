"""The OpenRMM MCP server.

Transport is streamable HTTP at /mcp. Authentication is an OpenRMM API key
...
from openrmm.mcp.context import access_token_for, authenticate
from openrmm.mcp.tools import register_tools

log = structlog.get_logger()

MCP_HOST = os.environ.get("OPENRMM_MCP_HOST", "0.0.0.0")
MCP_PORT = int(os.environ.get("OPENRMM_MCP_PORT", "8001"))
MCP_PATH = os.environ.get("OPENRMM_MCP_PATH", "/mcp")
NATS_CONNECT_TIMEOUT_S = float(os.environ.get("OPENRMM_MCP_NATS_TIMEOUT_S", "10"))

INSTRUCTIONS = """\
OpenRMM manages a fleet of enrolled machines (Windows, macOS, Linux), each
...
- Every call, successful or refused, is written to the OpenRMM audit log under
  the API key's name. Assume the user will read it.
"""


class ApiKeyVerifier(TokenVerifier):
    """Verifies `orpk_...` bearer tokens against the api_keys table.
...
def main() -> None:
    ...
    if not settings.mcp_enabled:
        print(
            "openrmm-mcp: refusing to start because OPENRMM_MCP_ENABLED is not true.\n"
            "The MCP server is opt-in: set OPENRMM_MCP_ENABLED=true in .env and run\n"
    ...
    print(
        f"openrmm-mcp v{__version__} on http://{MCP_HOST}:{MCP_PORT}{MCP_PATH} "
        "(bearer: an OpenRMM API key, orpk_...)",
        file=sys.stderr,
    )
    mcp.run(
        transport="http",
