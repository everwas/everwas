...
The key format is `orpk_<key_id>_<secret>`; only sha256(secret) is stored.
This lived in openrmm.mcp.context while MCP was the only surface that took
keys. The sync API authenticates with the same keys (exchanged for a
short-lived token, see openrmm.security.sync_tokens), so verification moved
