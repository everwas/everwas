...
| `agents.{id}.inventory.{kind}` | JetStream `INVENTORY` (per-subject max-msgs 1) | kind is one of `hardware`, `software`, `processes`, `services`, `patchstate`; full snapshot plus `snapshot_hash` |
